...
export default async function adminWorkRoutes(app) {
  ...
  app.get(`${ADMIN_BASE}/work/new`, { preHandler: gate }, async (req, reply) => {
    ...
    return renderHtml(reply, "admin/work-edit.eta", {
      ...
      user: req.currentUser,
      csrfToken,
      entry: { id: null, slug: "", title: "", kicker: "", eyebrow: "", lede: "", article_meta: "", body_html: "", cover_media_id: null, cover_filename: null, cover_alt: "", tags_csv: "", sort_order: 0, is_disabled: 0, meta_description: "", published: 1, published_at: new Date().toISOString().slice(0, 10) },
      media: listMedia(),
      mode: "new",
    });
  });
